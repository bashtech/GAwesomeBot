...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	const getTime = (location, member, otherusr) => {
		if(location) {
			unirest.get(`https://maps.googleapis.com/maps/api/geocode/json?key=${auth.tokens.google_maps_api_key}&address=${encodeURIComponent(location)}`).header("Accept", "application/json").end(res => {
				if(res.status==200 && res.body.results.length>0) {
					location = res.body.results[0].formatted_address;
					unirest.get(`https://maps.googleapis.com/maps/api/timezone/json?key=${auth.tokens.google_maps_api_key}&location=${res.body.results[0].geometry.location.lat},${res.body.results[0].geometry.location.lng}&timestamp=${Math.floor(Date.now() / 1000)}&sensor=false`).header("Accept", "application/json").end(res => {
